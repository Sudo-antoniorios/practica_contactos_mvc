...
class ContactoService
{
    ...
    {
        try {
            $q = isset($filtros['q']) && trim((string) $filtros['q']) !== '' ? trim((string) $filtros['q']) : null;
            $contactos = ($q !== null)
                ? $this->contactoModel->getByFilter($q)
                : $this->contactoModel->getAll();

            return array_map(fn(array $row): array => $this->mapearContacto($row), $contactos);
    ...
    {
        try {
            $this->contactoModel->setNombre((string) ($datos['nombre'] ?? ''));
            $this->contactoModel->setTelefono($datos['telefono'] ?? null);
            $this->contactoModel->setEmail($datos['email'] ?? null);

            if (!$this->contactoModel->set()) {
                throw new \RuntimeException('El modelo no pudo guardar el contacto.');
            }
    ...
    public function getUltimosContactos(int $limite = 3): array
    {
        try {
            ...
            return array_map(fn(array $row): array => $this->mapearContacto($row), $rows);
        } catch (DatabaseException $e) {
    ...
    private function mapearContacto(array $row): array
    {
        return [
            ...
            'created_at' => isset($row['created_at']) ? date('d/m/Y H:i', strtotime((string) $row['created_at'])) : 'No disponible',
            'created_at_raw' => $row['created_at'] ?? null,
            'updated_at' => isset($row['updated_at']) ? date('d/m/Y H:i', strtotime((string) $row['updated_at'])) : 'Sin cambios',
        ];
    }
}
